...
-/
import Champernowne.Count

/-!
# Interval ↔ digit-string counting (PLAN.md Goals 1a, 3, 4)
...
`CountExtras.lean`. All shapes validated empirically in `Sandbox.lean`
...
`CountExtras.lean`). (Validated by `#eval` in `Sandbox.lean`.) -/
...
theorem block_eq_iff {b m n : ℕ} (hb : 1 < b)
  ...
  have hsub : ((bigDigits b n).drop j).take w.length
    ...
    show (((Nat.digits b n).reverse.drop j).take w.length) = _
...
/-! ### Partial-cohort block occurrence sums (Goal 4)
...
/-! ### Partial-cohort comparison, boundary induction, and general `N` (Goal 4) -/
...
theorem le_base_pow_count_boundary {b : ℕ} (hb : 1 < b) (K : ℕ) {w : List ℕ}
  ...
  | succ K ih =>
    ...
    have hpow : b ^ (K + 1) = b * b ^ K := by rw [pow_succ']
    ...
    · -- cohort shorter than `w`: drop the count, bound the length crudely
      ...
      have hscale : 6 * (w.length + 1) * b ^ K + (w.length + 1) * b ^ (K + 1)
        ...
        rw [hpow]
        have hstep : (w.length + 1) * b ^ K * 2 ≤ (w.length + 1) * b ^ K * b :=
    ...
    · have hpart := le_base_pow_countOccurrences_partial hb K (b ^ (K + 1) - b ^ K)
      ...
      rw [hcov] at hpart
      have hscale : 6 * (w.length + 1) * b ^ K + 3 * (w.length + 1) * b ^ (K + 1)
          ≤ 6 * (w.length + 1) * b ^ (K + 1) := by
        rw [hpow]
        have hstep : (w.length + 1) * b ^ K * 2 ≤ (w.length + 1) * b ^ K * b :=
          Nat.mul_le_mul_left _ hb
