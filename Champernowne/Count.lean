...
/-!
# Occurrence-counting mini-library (PLAN.md Goal 2)

The `champernowne_normal`-facing core: behavior of `countOccurrences`
under `cons` and `++` (the lower append bound
`add_countOccurrences_le_append`), the position characterization, and the
`allWords` enumeration of all length-`k` digit strings with its window
pigeonhole (`sum_countOccurrences_allWords_le`), which lets upper
occurrence bounds be derived from lower ones by complement. The window
convention is documented at the definition in `Defs.lean`: the countable
windows are exactly the start positions `0 … l.length − w.length`, and
`w ≠ []` is assumed wherever the empty word degenerates. All statements
are two-sided ℕ inequalities with explicit constants — no asymptotics
here.

The rest of the Goal 2 mini-library (append UPPER sandwich with its sharp
`min` seam, append right-monotonicity, take/drop monotonicity, flatten
lower bound, `List.count` bridge) is not needed by the main theorem and
lives in `CountExtras.lean` at the project root, outside this library's
import graph.
-/
...
/-! ### Occurrences in `champBlocks` (Goal 2 ⇒ stream-level lower bound;
the upper direction is obtained downstream by the `allWords` pigeonhole,
so no seam-error upper bound is needed at the stream level) -/
