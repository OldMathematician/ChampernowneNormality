...
/-!
# ℝ layer and endgame (PLAN.md Goal 5)

`champM b n` is the digit length of the straddling number
`champIndex b n + 1`; this file is the `Nat.log` quarantine (CLAUDE.md
rule 8). The ℕ-layer transfer bounds become
`(countOccurrences w (champPrefix b n) − n/b^k) =o[atTop] n`, which
bridges to the frequency limit
`countOccurrences w (champPrefix b n) / n → b⁻ᵏ`.
-/
...
theorem tendsto_champM_atTop {b : ℕ} (hb : 1 < b) :
  ...
  show K ≤ Nat.log b (champIndex b n + 1) + 1
  omega

/-! ### General-`w` packaging (Goal 5)
...
theorem tendsto_err_div_pow {b : ℕ} (hb : 1 < b) (w : List ℕ) :
  ...
  have hg : Tendsto (fun n => (2 * (7 * ((w.length : ℝ) + 1) * (b : ℝ) ^ (2 * w.length) * (b : ℝ) ^ 2))
...
/-- Goal 5 packaging, general `w`: `countOccurrences w (champPrefix b n) −
n/b^k = o(n)`. -/
...
/-- Goal 5, headline (general `w`): every nonempty block `w` (digits `< b`)
