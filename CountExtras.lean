...
/-!
# Occurrence-counting extras — NOT required for `champernowne_normal`

Nothing in this file is reachable from the main theorem; it is imported
only by `Sandbox.lean`, never by the `Champernowne` library. It collects

* the remainder of the PLAN.md Goal 2 mini-library — the append UPPER
  sandwich with its sharp `min` seam, append right-monotonicity,
  take/drop monotonicity, the flatten lower bound, and the `List.count`
  bridge (the main proof needs only the lower append bound
  `add_countOccurrences_le_append`, in `Count.lean`; upper occurrence
  bounds are derived there via the `allWords` pigeonhole instead), and
* the EXACT interval ↔ digit-string counting layer of Goals 1a/3 —
  `digitEquiv` with its round-trips, the exact periodic count, the exact
  per-position cards (head, interior, past-end) and the exact cohort
  occurrence sums. The main path (`DigitCount.lean`) only ever consumes
  lower bounds: it drops occurrences at the head of a number (so no
  `w.head? = some 0` case split) and treats full cohorts as partial
  cohorts, and
* `champPrefix_prefix_champBlocks` (Goal 4), the second half of the
  prefix chain `champBlocks (champIndex n) <+: champPrefix n <+:
  champBlocks (champIndex n + 1)`; the derived upper transfer made its
  former uses in `Positions.lean` obsolete.

Kept as documented API and as Mathlib upstream candidates. Imports
`Champernowne.Positions` (rather than just `DigitCount`) to reach
`champPrefix_prefix`/`champBlocks_prefix` for the last of these.
-/
...
/-- The PLAN.md form of the seam bound. -/
...
/-! ### Exact interval ↔ digit-string counting (Goals 1a, 3)
...
(Validated by `#eval` in `Sandbox.lean`.) -/
theorem card_Ico_filter_div_mod {A B p q d : ℕ} (hp : 0 < p) (hq : 0 < q)
  ...
  · -- forward map lands in the box
    intro n hn
    simp only [Finset.coe_filter, Set.mem_setOf_eq, Finset.mem_Ico] at hn
    obtain ⟨⟨h1, h2⟩, _⟩ := hn
...
NOT required for `champernowne_normal` (only `digitEquiv` uses it); kept
with round-trip 2 as documented Goal 1a API. -/
...
the number it denotes. (Holds vacuously for `l = []` as well.)
NOT required for `champernowne_normal` (only `digitEquiv` uses it). -/
...
`n / b^(m-1-j) % b`. -/
theorem getElem?_bigDigits {b m n : ℕ} (hb : 1 < b)
  ...
  show (Nat.digits b n).reverse[j]? = _
...
NOT required for `champernowne_normal`: the proof only ever consumes
lower-bound `Finset.card` consequences, never the equivalence itself.
Kept as the Goal 1a deliverable, a `Sandbox.lean` test subject, and a
...
theorem sum_countOccurrences_cohort_zero {b : ℕ} (hb : 1 < b) (m : ℕ)
  ...
  have htail : ∑ j ∈ Finset.Ico (m - w.length + 1) (m + 1),
    ...
    exact card_blockAt_past_end hb m j hwne (by omega)
  rw [htail, Nat.add_zero, ← Finset.range_eq_Ico, Finset.sum_range_succ',
    card_blockAt_head_zero hb m hw hw0 hk, Nat.add_zero]
  have hint : ∀ i ∈ Finset.range (m - w.length),
      ((Finset.Ico (b ^ (m - 1)) (b ^ m)).filter
        (fun n => ((bigDigits b n).drop (i + 1)).take w.length = w)).card
      = (b - 1) * b ^ (m - w.length - 1) := by
...
NOT required for `champernowne_normal` (the derived upper transfer made
its former uses obsolete); kept as the second half of the documented
prefix chain `champBlocks (champIndex n) <+: champPrefix n <+:
